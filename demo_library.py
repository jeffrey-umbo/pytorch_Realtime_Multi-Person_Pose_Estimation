...
class ConnectAndVisualize():
    def __init__(self):
        self.limbSeq = [[2, 3], [2, 6], [3, 4], [4, 5], [6, 7], [7, 8],
    ...
    def connect(self, paf_avg, all_peaks, oriImg, param_):
        ...
        the second last number in each row is the score of
    ...
    def display(self, oriImg, all_peaks, subset, candidate):
        ...
        for n in range(len(subset)):
            for i in range(17):
                ...
                canvas = self.draw_limb(X, Y, canvas, i)
                
            for (p1, p2) in modify_part:             
                index1 = subset[n][np.array(self.limbSeq[p1])-1]
                index2 = subset[n][np.array(self.limbSeq[p2])-1]
                if -1 not in index1 and -1 not in index2:
                    Y1 = candidate[index1.astype(int), 0]
                    X1 = candidate[index1.astype(int), 1]
                    Y2 = candidate[index2.astype(int), 0] 
                    X2 = candidate[index2.astype(int), 1]
                    if p1 == 6:
                        Y = (Y1+Y2)/2
                        canvas = self.draw_limb(X1, Y, canvas, p1)
                    ...
                    if p1 == 7:
                        ...
                        canvas = self.draw_limb(mx1, my1, canvas, p1)
                        canvas = self.draw_limb(mx2, my2, canvas, p2)
    ...
    def draw_limb(self, X, Y, canvas, i):
    ...
    def __call__(self, paf_avg, all_peaks, oriImg, param_):
        ...
        return self.display(oriImg, all_peaks, subset, candidate)
...
class HeatmapPafGenerator():
    ...
    def __call__(self, oriImg):
        ...
        imageToTest_padded, pad = \
            util.padRightDownCorner(imageToTest, self.model_['stride'], self.model_['padValue'])
        ...
        heatmap_avg = T.transpose(
            T.transpose(heatmap[0], 0, 1), 1, 2
        ).data.cpu().numpy()
        paf_avg = T.transpose(
            T.transpose(paf[0], 0, 1), 1, 2
        ).data.cpu().numpy()
